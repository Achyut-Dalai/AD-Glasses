...
The retired Android/CyanBridge-derived application is **not a reference implementation** for this app.

Do not copy, port, infer, or restore architecture, UI, state handling, feature behavior, protocol assumptions, media contracts, speech choices, assets, package structure, or helper code from the retired Android application or from its historical branches/commits. Its old IPFS-style media assumptions are retired as well.

Allowed implementation references are:

1. the native iOS app on `main` for product behavior, feature intent, settings/provider semantics, and visual/product direction;
2. physical captures and the verified HeyCyan reverse-engineering/official-app research in `docs/heycyan/` for hardware/protocol behavior;
3. retained official/vendor SDK artifacts when they provide independently useful evidence;
4. current Android/platform/library documentation and fresh technical research when improving beyond the iOS baseline.

Android should preserve proven iOS behavior where it is good, but it is explicitly allowed to exceed iOS where Android permits a better implementation. Improvements must be justified by current platform behavior, measurements, or verified protocol evidence—not by resurrecting the retired Android app.

## Source-of-truth order

For product behavior and UX, start from the native iOS app and improve deliberately. For hardware/protocol claims, when evidence conflicts, prefer:

1. physical hardware captures and official production-app behavior;
2. official/vendor SDK interfaces and binaries;
3. documented reverse-engineering findings in `docs/heycyan/`;
4. current platform/library documentation and reproducible fresh experiments;
5. our own assumptions.

Never promote an assumption into protocol truth. Never use the retired Android/CyanBridge implementation to break a tie.
...
- iOS cloud speech baseline: Groq Whisper using `whisper-large-v3-turbo`; Android may add local-first recognition and use Groq as fallback/verification rather than replacing that proven provider contract with an unrelated default;
