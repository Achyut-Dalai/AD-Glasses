...
This document turns AD Glasses's existing local-agent groundwork into a concrete MVP for AI-driven phone control via Android Accessibility.
...
- Prefer implementing our own Kotlin-first version on top of AD Glasses's existing local-agent code.
...
## Current AD Glasses Baseline

AD Glasses already has more groundwork than a blank MVP:

- Accessibility service exists: `app/src/main/java/com/ad_glasses/localagent/accessibility/LocalAgentAccessibilityService.kt`
- Foreground agent loop exists: `app/src/main/java/com/ad_glasses/localagent/LocalAgentService.kt`
...
Keep AD Glasses's existing service-based architecture and add a dedicated UI-control loop on top.
...
- Use the existing network-backed AI path already present in AD Glasses for the MVP.
...
This is where AD Glasses should be stricter than the inspiration repo.
...
Build this as a native AD Glasses Kotlin feature, borrowing `private-agent`'s best idea: the small, repeated observe -> decide one action -> execute -> observe loop.
...
- chat-driven entrypoint inside AD Glasses
