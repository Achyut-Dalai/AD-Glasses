...
actual class PlatformHttpClient actual constructor() {
    ...
    actual suspend fun postMultipart(
        ...
        val boundary = "----ADGlassesBoundary${System.currentTimeMillis()}"
        val conn = URL(url).openConnection() as HttpURLConnection
        try {
            conn.requestMethod = "POST"
            conn.doOutput = true
    ...
    private fun readResponse(conn: HttpURLConnection): HttpResponse {
        ...
        return HttpResponse(statusCode = code, headers = conn.headerFields, body = body, bodyBytes = body.toByteArray())
    }
}
