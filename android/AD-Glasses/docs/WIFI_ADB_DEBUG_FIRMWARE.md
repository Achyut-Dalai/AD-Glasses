...
**Date:** 2026-07-16
**Author:** AD Glasses Hardware Team
...
The file `AD-Glasses_wifi_chip_debug.swu` is a patched firmware image intended for the WIFIAM01G1 glasses family. It is intended to enable ADB (Android Debug Bridge) over Wi-Fi, but neither boot-time Wi-Fi association nor TCP ADB has been validated on hardware.
...
The original `AD-Glasses_wifi_chip_debug.swu` remains rejected. A separate corrected candidate was rebuilt from the exact stock SWU without the `HeyCyanDebug` hotspot script. It changes only `/etc/init.d/adbd` in each rootfs:
...
1. Install a debug AD Glasses build and use only a recoverable lab device.
...
4. In AD Glasses, open Glasses → Developer tools → ADB over glasses Wi-Fi Direct.
5. Read and explicitly acknowledge the destructive-risk warning.
6. AD Glasses registers the BLE IP listener and starts reset-disabled P2P discovery.
7. AD Glasses sends exactly one confirmed transfer-entry command `[0x02,0x01,0x04]`.
8. The production transfer-mode path loads WLAN, starts P2P, and reports the glasses IP through notify `0x08`.
9. AD Glasses probes `<glasses-ip>:5555` through the selected P2P `Network.socketFactory`.
10. If reachable, AD Glasses starts a loopback-only phone relay on `127.0.0.1:15555`.
...
12. Press Stop before leaving the screen. AD Glasses closes the relay, sends confirmed exit-transfer command `[0x02,0x01,0x09]` at most once after entry acknowledgement, and tears down P2P.
...
1. **Wi-Fi OTA (lab path, not yet hardware-validated):** AD Glasses can serve the SWU to the glasses over P2P. The package instructs `swupdate` to rewrite the selected NOR or SDNAND image set; do not infer that both rootfs copies are flashed in one run.
...
USB tethering is not a reliable substitute. It normally creates a second downstream subnet and Android commonly blocks forwarding between USB-tethered clients and Wi-Fi-hotspot clients. `bindProcessToNetwork()` in AD Glasses affects only that Android app and does not create a PC-to-glasses route.
...
`AD-Glasses_wifi_chip_debug.swu` is based on `WIFIAM01G1_1.00.28_2603031800.swu` and must be treated as limited to the exact matching **WIFIAM01G1** hardware/base version. Compatibility has not been established by a device flash.
...
| Field | Value |
|---|---|
| Filename | `AD-Glasses_wifi_chip_debug.swu` (rejected artifact; do not flash) |
...
The following enhancements are being considered and may be forwarded to the AD Glasses engineering team:
