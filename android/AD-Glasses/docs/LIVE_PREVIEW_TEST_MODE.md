...
The current AD Glasses implementation deliberately does not trigger this mode. It arms BLE/P2P discovery and probes the RTSP stream after the hardware team activates mode 8 through an independently verified lab procedure.
...
The following items are unclear and will need to be resolved during field testing. Doubts may be forwarded to the AD Glasses engineering team for clarification:
...
### Prerequisites
- Glasses connected to phone via BLE (standard AD Glasses app connection)
...
2. Open a debug AD Glasses build and navigate to the Glasses dashboard.
...
| File | Purpose |
|---|---|
| `app/src/main/java/com/ad_glasses/ota/LivePreviewManager.kt` | Core logic: passive BLE notification listener, P2P, IP discovery, RTSP probe, ExoPlayer |
...
| `app/src/main/java/com/ad_glasses/MainActivity.kt` | Action wiring, dialog display |
