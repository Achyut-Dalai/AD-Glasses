    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                ...
                actions = {
                    IconButton(onClick = vm::newConversation) { Icon(Icons.Outlined.Edit, contentDescription = "New conversation") }
                },
            )
        },
        bottomBar = {
        ...
        if (messages.isEmpty()) {
            AssistantWelcome(Modifier.padding(inner))
        } else {
            LazyColumn(
                state = listState,
                ...
                item {
                    Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                        ...
                        Text("A reading-first transcript — no bubble wall.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
...
@Composable
private fun AssistantWelcome(modifier: Modifier = Modifier) {
    Column(
