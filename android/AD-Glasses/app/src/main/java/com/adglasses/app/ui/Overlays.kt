        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                Row(
                    ...
                    LanguageButton(source) { source = nextLanguage(source) }
                    Text("→")
                    LanguageButton(target) { target = nextLanguage(target) }
                }
...
@Composable
private fun LanguageButton(tag: String, onClick: () -> Unit) {
    OutlinedButton(onClick = onClick) { Text(languageName(tag)) }
}

private val languages = listOf("en", "hi", "es", "fr", "de", "pt", "ru", "ar", "ja", "ko", "zh")
