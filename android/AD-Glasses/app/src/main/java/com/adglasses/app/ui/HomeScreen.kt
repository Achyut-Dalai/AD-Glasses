...
fun HomeScreen(
    ...
    requestCommunicationAccess: () -> Unit,
    companionLink: CompanionLinkState,
    changeBackgroundLink: () -> Unit,
) {
    val connection by vm.glasses.collectAsStateWithLifecycle()
    val backgroundDetail = when (companionLink) {
        CompanionLinkState.Unsupported -> "Not supported on this Android device"
        CompanionLinkState.Available -> "Enable OS-level presence recovery"
        CompanionLinkState.Associating -> "Waiting for Android approval"
        is CompanionLinkState.Linked -> "Enabled • tap to unlink"
        is CompanionLinkState.Failed -> "Retry OS-level presence recovery"
    }
    val actions = listOf(
        ...
        HomeAction("Phone & text", "Enable contact calls and direct SMS", Icons.Filled.PhoneInTalk, requestCommunicationAccess),
        HomeAction("Background link", backgroundDetail, Icons.Filled.Sync, changeBackgroundLink),
    )
