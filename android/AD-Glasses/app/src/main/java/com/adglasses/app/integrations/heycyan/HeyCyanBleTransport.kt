...
class HeyCyanBleTransport(private val context: Context) {
        ...
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            when (newState) {
                BluetoothProfile.STATE_CONNECTED -> {
                    ...
                    if (!gatt.discoverServices()) onEvent?.invoke(Event.Error("Could not discover glasses services"))
                }
    ...
    private fun enableNextNotification(gatt: BluetoothGatt) {
        ...
        if (Build.VERSION.SDK_INT >= 33) {
            ...
            run {
                ...
                if (!gatt.writeDescriptor(descriptor)) onEvent?.invoke(Event.Error("Could not write a glasses notification descriptor"))
            }
        }
    }
    ...
    private fun write(characteristic: BluetoothGattCharacteristic?, bytes: ByteArray): Boolean {
        ...
        return if (Build.VERSION.SDK_INT >= 33) {
            currentGatt.writeCharacteristic(target, bytes, BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE) == android.bluetooth.BluetoothStatusCodes.SUCCESS
        } else {
            @Suppress("DEPRECATION")
            run {
