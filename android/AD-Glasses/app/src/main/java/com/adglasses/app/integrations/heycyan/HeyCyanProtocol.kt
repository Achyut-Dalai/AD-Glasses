...
object HeyCyanResponseDecoder {
    ...
    fun networkPreparation(frame: HeyCyanFrame): HeyCyanNetworkPreparation {
        ...
        val mode = HeyCyanNetworkMode.fromWire(p[3].u8()) ?: error("Glasses rejected or returned unknown network mode")
    ...
    fun deviceEvent(frame: HeyCyanFrame): HeyCyanDeviceEvent {
        ...
        return when (p[0].u8()) {
            ...
            0x03 -> if (p.size >= 2 && p[1].u8() == 1) HeyCyanDeviceEvent.AssistantListeningStarted else HeyCyanDeviceEvent.Unknown(0x03, p.copyOf())
            0x05 -> {
                require(p.size >= 3)
                val level = p[1].u8()
                val charging = p[2].u8()
            ...
            0x08 -> {
                require(p.size >= 5)
                HeyCyanDeviceEvent.WifiAddress(p.copyOfRange(1, 5).joinToString(".") { it.u8().toString() })
            }
            0x09 -> {
                require(p.size >= 2)
                HeyCyanDeviceEvent.WifiError(p[1].u8())
            }
            0x0A -> if (p.size >= 2 && p[1].u8() == 1) HeyCyanDeviceEvent.AssistantListeningEnded else HeyCyanDeviceEvent.Unknown(0x0A, p.copyOf())
...
fun HeyCyanCommand.matches(frame: HeyCyanFrame): Boolean {
    ...
    return when (this) {
        ...
        HeyCyanCommand.StartTranslationHeartbeat, HeyCyanCommand.StopTranslationHeartbeat -> frame.payload.size >= 3 && frame.payload[1].u8() == 0x0C
    }
}
...
private fun buildTimeSyncRecord(
    ...
    return byteArrayOf(
        bcd(local.year % 2000), bcd(local.monthValue), bcd(local.dayOfMonth),
        bcd(local.hour), bcd(local.minute), bcd(local.second),
        languageCode(locale), timezone.toByte(), 0x01,
    )
}
...
private fun languageCode(locale: Locale): Byte {
    val code = when (locale.language.lowercase(Locale.ROOT)) {
        ...
        "en" -> 1; "el" -> 3; "fr" -> 4; "de" -> 5; "it" -> 6; "es" -> 7
        "nl" -> 8; "pt" -> 9; "ru" -> 10; "tr" -> 11; "ja" -> 12; "ko" -> 13
        "pl" -> 14; "ro" -> 15; "ar" -> 16; "th" -> 17; "vi" -> 18
        "id", "in" -> 19; "hi" -> 20; "cs" -> 21; "sk" -> 22; "hu" -> 23
        "he", "iw" -> 24; "hr" -> 25; "sl" -> 26
        else -> 1
    }
    return code.toByte()
}
