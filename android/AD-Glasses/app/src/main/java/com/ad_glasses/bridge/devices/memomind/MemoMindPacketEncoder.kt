...
object MemoMindPacketEncoder {
    ...
    fun encodeShowText(text: String): ByteArray {
        return encodeScheduleItems(
            listOf(
                ScheduleItem(title = text.trim().ifEmpty { "ADGlasses" }),
            ),
        )
    }
    ...
    fun encodeLines(lines: List<String>): ByteArray {
        ...
        return encodeScheduleItems(items.ifEmpty { listOf(ScheduleItem(title = "ADGlasses")) })
    }

    fun encodeScheduleCard(title: String, body: String): ByteArray {
        return encodeScheduleItems(listOf(ScheduleItem(title = title.ifBlank { "ADGlasses" }, body = body)))
    }
    ...
    fun encodeNotification(
        title: String,
        body: String,
        appName: String = "ADGlasses",
        packageName: String = "com.ad_glasses",
    ): ByteArray {
