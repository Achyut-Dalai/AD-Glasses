...
import com.ad_glasses.data.local.entity.MemoryChunk
import com.ad_glasses.data.local.entity.MemoryChunkSources
import com.ad_glasses.memoryvault.MemoryPolicyService
import com.ad_glasses.memoryvault.MemoryRefMapper
...
object LocalAgentMemoryRoomIndex {
    ...
    ): List<com.ad_glasses.data.local.dao.MemoryChunkDao.SearchHit> {
        if (VaultLockStateManager.isLocked(context)) return emptyList()
        val q = toFtsQuery(query)
        if (q.isBlank()) return emptyList()
        val mode = com.ad_glasses.memoryvault.MemoryModeManager.getSelectedMode(context)
