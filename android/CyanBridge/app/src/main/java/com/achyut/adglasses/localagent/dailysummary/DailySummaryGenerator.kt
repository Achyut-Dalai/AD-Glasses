...
object DailySummaryGenerator {
    ...
    fun providerHint(context: Context): String {
        return when (AutomationPrefs.getProviderType(context)) {
            ...
            AgentProviderType.CLOUD_API -> "cli_cloud"
            AgentProviderType.TASKER -> "cli_cloud"
        }
    }
...
""".trim()
            ...
            AgentProviderType.LOCAL_AGENT -> {
                runCatching { runLocalModels(context, prompt) }
                    .recoverCatching { localErr ->
                        runCloud(context, prompt)
                    }
                    .getOrThrow()
            }

            AgentProviderType.CLOUD_API -> runCloud(context, prompt)

            AgentProviderType.TASKER -> runCloud(context, prompt)
        }
    }

    private suspend fun runCloud(context: Context, prompt: String): ProviderResponse {
        val modelOverride = AiPrefs.getTasksModel(context)
            .trim()
            .takeIf { it.isNotBlank() }

        val details = CliCloudClient.voiceQueryDetailed(
            context = context,
            prompt = prompt,
            modelOverride = modelOverride,
        ).getOrElse { err ->
            throw IllegalStateException("Cloud provider unavailable (${err.message}).")
        }
    ...
    private suspend fun runRouterFallback(context: Context, prompt: String): ProviderResponse {
        val inputTokens = DailySummaryRunHistory.estimateTokenCount(prompt)
        val started = System.currentTimeMillis()
        val reply = CliCloudRouter.textReply(context, prompt).trim()
