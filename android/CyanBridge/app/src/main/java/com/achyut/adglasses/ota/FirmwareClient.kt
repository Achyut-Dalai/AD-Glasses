...
/**
 * Client for the AdGlasses server's firmware download API.
 *
...
class FirmwareClient(
    ...
    fun fetchAndDownload(
        ...
        val apiToken = ServerPrefs.getApiToken(context)
        val accountEmail = ServerPrefs.getAccountEmail(context)
        if (apiToken.isBlank()) {
            Log.e(TAG, "[2/5] FAIL: No API token available. User not signed in.")
            return FirmwareResult.Error("No API token available. Please sign in to your AD Glasses account.")
        }
