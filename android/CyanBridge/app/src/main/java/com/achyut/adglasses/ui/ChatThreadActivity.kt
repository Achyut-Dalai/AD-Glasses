...
import com.achyut.adglasses.ai.router.CliCloudRouter
...
class ChatThreadActivity : AppCompatActivity() {
    ...
    override fun onDestroy() {
        ...
        if (localGenerationRunning) {
            lifecycleScope.launch {
                CliCloudRouter.cancelCurrentGeneration(this@ChatThreadActivity)
            }
        }
        super.onDestroy()
    }
    ...
    private fun showRelayModelPicker() {
        val current = AiPrefs.getRequestsModel(this)
        lifecycleScope.launch {
            val options = withContext(Dispatchers.IO) {
                val fetched = CliCloudClient.fetchAvailableModels(this@ChatThreadActivity).getOrElse { emptyList() }
            ...
            AlertDialog.Builder(this@ChatThreadActivity)
                ...
                .setItems(labels) { _, which ->
                    ...
                    AiPrefs.setRequestsModel(this@ChatThreadActivity, pickedId)
                    refreshModelBadge("Ready")
                    android.widget.Toast.makeText(
                        this@ChatThreadActivity,
    ...
    private fun isRelayProviderSelected(): Boolean {
        return AiProviderPrefs.getProvider(this) == AiProviderType.CLOUD_API
    }
    ...
    private fun refreshModelBadge(status: String) {
        modelBadge = when (AiProviderPrefs.getProvider(this)) {
            ...
            AiProviderType.CLOUD_API -> {
                val selected = AiPrefs.getRequestsModel(this).ifBlank { "auto" }
    ...
    private fun showRelayDownToastIfNeeded(message: String?) {
        val hint = CliCloudClient.relayUnavailableHintFromText(message.orEmpty()) ?: return
        ...
        lifecycleScope.launch {
            ...
            try {
                val assistantReply = withContext(Dispatchers.IO) {
                    ...
                    CliCloudRouter.chatReplyStreaming(
                        ...
                        callbacks = if (useLocalStreaming) {
                            object : CliCloudRouter.ChatStreamCallbacks {
                ...
                if (useLocalStreaming && DebugLogSupport.isLocalRuntimeIssue(t.message, t)) {
                    DebugLogSupport.showSupportOptionsDialog(
                        ...
                        description = "AD Glasses hit a local model runtime error while generating this reply.",
    ...
    private suspend fun attemptRepairDailyFactsUpdate(
        ...
        return runCatching {
            ...
            val repairedRaw = withContext(Dispatchers.IO) {
                CliCloudRouter.textReply(this@ChatThreadActivity, prompt)
            }
