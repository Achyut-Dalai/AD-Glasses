...
class LocalAgentTelegramService : Service() {
    ...
    private fun handleTask(goal: String): String {
        ...
        if (!AutomationPrefs.isLocalAgentAutomationEnabled(applicationContext)) {
            return "Phone control is disabled in AdGlasses settings."
        }
        ...
        return if (result.ok) {
            "Task accepted. AdGlasses will require any approvals configured on the phone."
        } else {
            "Task was not started. Check AdGlasses Local Agent settings on the phone."
        }
    }

    private fun handleStop(): String {
        LocalAgentController.stop(applicationContext)
        return "Stop request sent to AdGlasses."
    }
    ...
    private suspend fun handleReadScreen(): String {
        ...
        if (!hasNotificationPermission(this) || !LocalAgentAccessibilityBridge.isConnected()) {
            return "Screen reading is unavailable. Check AD Glasses Accessibility and notification access on the phone."
        }
        ...
        return if (executed) {
            "AdGlasses is reading the current screen aloud."
        } else {
            "Screen reading is waiting for approval in AdGlasses."
        }
    }
