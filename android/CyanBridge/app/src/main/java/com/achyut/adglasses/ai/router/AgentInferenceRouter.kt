...
object AgentInferenceRouter {
    ...
    suspend fun completeUiPlanning(
        ...
        val imageContent = try {
            when (providerType) {
                ...
                AgentProviderType.CLOUD_API -> CliCloudClient.imageQuery(
                    context = context,
                    imagePath = usableImagePath,
                    prompt = multimodalPrompt(systemPrompt, userPrompt),
                    modelOverride = AiPrefs.getTasksModel(context),
                ).getOrThrow()

                AgentProviderType.TASKER -> when (AiProviderPrefs.getProvider(context)) {
                    AiProviderType.CLOUD_API -> CliCloudClient.imageQuery(
                        context = context,
                        imagePath = usableImagePath,
                        prompt = multimodalPrompt(systemPrompt, userPrompt),
                    ).getOrThrow()
    ...
    private suspend fun completeText(
        ...
        return when (providerType) {
            ...
            AgentProviderType.CLOUD_API -> CliCloudClient.chat(
                context = context,
                chatId = sessionId,
                prompt = userPrompt,
                messages = messages,
    ...
    private suspend fun completeUsingAiProviderPrefs(
        ...
        return when (AiProviderPrefs.getProvider(context)) {
            AiProviderType.CLOUD_API -> CliCloudClient.chat(
                context = context,
                chatId = sessionId,
                prompt = userPrompt,
                messages = messages,
