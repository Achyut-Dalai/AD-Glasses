...
import com.achyut.adglasses.plugins.walkingaid.vision.DepthResult
...
import com.achyut.adglasses.plugins.walkingaid.vision.VisionBackend
...
class WalkingAidService : Service() {
    ...
    private fun startLoop() {
        ...
        if (!isMetaRayban && !BleOperateManager.getInstance().isConnected) {
            ...
            showToast(this, getString(com.achyut.adglasses.R.string.walking_aid_not_connected))
            RUNNING.set(false)
        ...
        if (MeetingCapturePrefs.getState(this).isRecording) {
            ...
            showToast(this, getString(com.achyut.adglasses.R.string.walking_aid_meeting_active))
            RUNNING.set(false)
            stopForeground(STOP_FOREGROUND_REMOVE)
            stopSelf()
            return
        ...
        visionWorkerJob = scope.launch {
            ...
            for (frame in frameChannel) {
                ...
                val detectionResult = if (imageSource == "cloud" && !frame.sourcePath.isNullOrBlank() && File(frame.sourcePath).exists()) {
                    ...
                    val cloudReply = CliCloudClient.imageQuery(this@WalkingAidService, frame.sourcePath, prompt, modelOverride = modelOverride).getOrDefault("")
                ...
                if (WalkingAidPreferences.isDepthEnabled(this@WalkingAidService)) {
                    if (depthSource == "cloud" && !frame.sourcePath.isNullOrBlank() && File(frame.sourcePath).exists()) {
                        ...
                        val cloudDepthReply = CliCloudClient.imageQuery(this@WalkingAidService, frame.sourcePath, depthPrompt, modelOverride = depthModelOverride).getOrDefault("")
                ...
                if (warningDecision.shouldWarn && spokenMessage.isNotBlank()) {
                    ...
                    if (stateSource == "cloud") {
                        val cloudLlmReply = CliCloudClient.voiceQuery(this@WalkingAidService, scenePrompt).getOrNull()
    ...
    companion object {
        ...
        const val ACTION_START = "com.achyut.adglasses.action.WALKING_AID_START"
        const val ACTION_STOP = "com.achyut.adglasses.action.WALKING_AID_STOP"
