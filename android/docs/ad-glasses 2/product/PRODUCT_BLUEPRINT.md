...
This document defines the intended product experience. Visual references and generated prototypes may explore execution, but they must not change the information architecture, remove supported functionality, invent device behavior, or introduce monetization.
...
Think in **12 reusable screen families**, not one screen for every backend class:

| Family | Contains |
|---|---|
| Onboarding and Devices | Welcome, readiness, scan, confirmation and connection steps |
| Home | Primary destination and device/activity states |
| Assistant | Hub, conversation, visual/grounded/live/action states |
| Library | Primary destination plus adaptable Content Detail |
| Automations | Catalog, shared details and secondary Community flows |
| Device Center | Capability-led controls and family modules |
| Sync | Local media transfer and recovery |
| Settings | Short configuration index |
| AI Services | Local, owner-cloud, web-grounding and routing configuration |
| Privacy and Data | Memory, inventory, retention, vault and data actions |
| HeyCyan Firmware Lab | Experimental preflight, paired update and recovery |
| Advanced and Diagnostics | Expert tools, logs and prototype runtimes |

The deeper page specifications sometimes split a family into separate route templates—for example Assistant hub versus a focused conversation, or Library versus Content Detail. That is an implementation detail, not another top-level page. State variants are not new pages. The prototype reuses the same layouts and components instead of creating dozens of unrelated static screens.
...
| Meta Ray-Ban | **Experimental Android integration** | DAT registration, device availability, session, camera/photo/stream, and display adapter paths exist; expose only capabilities confirmed by the installed SDK/runtime and retain Experimental status until physical validation |
...
The identity is **mark-first**, with a compact approved symbol-and-wordmark lockup where the product identity belongs. The Home app bar may use that restrained lockup, but never a separate logo beside a repeated or oversized plain-text “AD GLASSES” heading. The symbol alone is used when space is tight; the fuller wordmark is also used on Welcome, About, and brand/export surfaces.

If the identity is not yet approved, explore four professional black-and-white symbol candidates across two families:
...
- Family-specific modules:
  ...
  - Meta Ray-Ban experimental DAT registration, availability, session, stream, photo, and display controls only when the installed SDK/runtime reports the corresponding capability; physical validation remains pending.
...
Frames: HeyCyan-compatible primary, Eyevue experimental, Meizu experimental, Meta experimental runtime-capability, and generic/unknown examples.
...
The prototype and production implementation should name and reuse these components:
...
- Original CyanBridge name, logo, author identity, or author-hosted AI assumptions.
...
- Preserve proven protocol code behind interfaces; do not rewrite it from visual references or AI Studio output.
...
Do not connect the complete visual rewrite to production hardware/backend behavior until the following are accepted in the installable prototype:

1. One shared light-only design system.
2. One coherent Product Stage/Home direction; no additional competing Home directions.
3. The same Home layout handles not-set-up, connecting, connected, reconnecting, disconnected, recording, syncing, firmware-active, and recoverable-error states.
4. The four-tab navigation shell and reusable focused-detail families.
5. All nine journeys in the canonical UI spec work with deterministic fixture state, including failure and recovery.
6. Reusable components and assets with editable text and layers.
7. Prototype controls, consistency review, physical-phone review, and accessibility audit pass.

The concise current UI contract is in `../design/CANONICAL_UI_SPEC.md`. The machine-readable visual identity is in `../design/DESIGN_SYSTEM.md`. Historical Stitch prompts are retained under `../archive/` and are not authoritative.
...
| External assistant/Tasker setup | AI Services & Models → External assistant (advanced) |
...
| Meta Ray-Ban registration plus runtime-gated session/stream/photo/display adapter paths | Device Center experimental module; unavailable capabilities are omitted or explicitly explained and physical validation remains pending |
