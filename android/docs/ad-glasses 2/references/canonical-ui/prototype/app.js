...
screens.advanced = () => shell('', `${topbar('Advanced', { detail: true, back: 'settings' })}<section class="content">
  ...
  <div class="section-title"><h2>External runtimes</h2></div><div class="card settings-list"><div class="row"><div class="leading-icon neutral">${icon('assistant')}</div><div class="grow"><div class="row-title">Phone automation bridge</div><div class="row-subtitle">Local Agent and Accessibility readiness</div></div><span class="chip">Prototype</span></div><div class="row divided"><div class="leading-icon neutral">${icon('globe')}</div><div class="grow"><div class="row-title">Display runtimes</div><div class="row-subtitle">EvenHub, Mentra relay and research adapters</div></div><span class="chip">Research</span></div></div>
</section>`, { focused: true, header: false });
