...
private struct LiveTranslateExperience: View {
    ...
    private var groqSettings: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Whisper transcribes each isolated speech turn so you can verify exactly what it heard. Your selected Groq Cloud AI model then translates that recognized text into English, including short one- and two-word turns. Whisper direct audio translation is only a fallback. Apple Translation is not used in the Groq path.")
                .font(.footnote)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
    ...
    private var liveTranslationCard: some View {
        TranslateCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text(selectedEngine == .groq
                     ? "AD listens for one phrase, closes the microphone, lets Whisper recognize that isolated audio, translates the recognized text with Groq AI, speaks the English result, then opens a fresh microphone turn. It never listens to its own translated speech."
